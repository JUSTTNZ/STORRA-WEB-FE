        ...
        <div className="space-y-3">
          {lessons.length > 0 ? (
            lessons.map((lesson, index) => {
              const lessonId = lesson.lessonId || lesson.id || lesson._id;
              const status = lesson.status || (lesson.completed ? 'completed' : 'not_started');
              const progress = lesson.progress || (lesson.completed ? 100 : 0);

              return (
                <div
                  key={lessonId || index}
                  onClick={() => handleLessonClick(lesson)}
                  className={`p-4 rounded-xl border cursor-pointer transition-all hover:shadow-md ${getStatusColor(status)}`}
                >
                  <div className="flex items-center gap-4">
                    ...
                    {/* Lesson Number */}
                    <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-[var(--secondary-100)] dark:bg-[var(--secondary-700)] flex items-center justify-center text-sm font-medium text-[var(--secondary-600)] dark:text-[var(--text-muted)]">
                      {lesson.order || index + 1}
                    </div>

                    {/* Lesson Info */}
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-[var(--secondary-800)] dark:text-[var(--text)] line-clamp-1">
                        {lesson.title || lesson.name || `Lesson ${index + 1}`}
                      </h3>
                      <div className="flex items-center gap-3 text-xs text-[var(--secondary-500)] dark:text-[var(--text-muted)] mt-1">
                        <span className="flex items-center gap-1">
                          {getLessonIcon(lesson)}
                          {lesson.duration || '10 mins'}
                        </span>
                        {status === 'in_progress' && (
                          <span className="text-[var(--primary-400)] dark:text-[var(--primary)]">
                            {progress}% complete
                          </span>
                        )}
                        {lesson.isBookmarked && (
                    ...
                    {/* Arrow */}
                    <ChevronRight className="w-5 h-5 text-[var(--secondary-400)] dark:text-[var(--secondary-500)] flex-shrink-0" />
                  </div>
        ...
        <div className="bg-white dark:bg-[var(--card-background)] rounded-xl border border-[var(--secondary-100)] dark:border-[var(--border-color)] p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-[var(--secondary-50)] dark:bg-[var(--secondary-800)] rounded-lg p-3 text-center">
              ...
              <p className="text-lg font-bold text-[var(--secondary-800)] dark:text-[var(--text)]">
                {courseData.quiz.timeLimit || 15} mins
              </p>
            </div>
            <div className="bg-[var(--secondary-50)] dark:bg-[var(--secondary-800)] rounded-lg p-3 text-center">
